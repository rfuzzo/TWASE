    ...
    try
    {
        ...
        if (err)
        {
            ...
            SHOW_MESSAGE_BOX_AND_EXIT_FILE_LINE(
                "An error occurred while checking logs directory existence:\n{}\n\nDirectory: {}", Utils::Widen(msg),
                dir.c_str());
...
std::wstring Utils::GetStateName(EGameStateType aStateType)
{
    using enum EGameStateType;
    switch (aStateType)
    {
...
{
    return ShowMessageBoxEx(L"TWASE", aText, aType);
}
