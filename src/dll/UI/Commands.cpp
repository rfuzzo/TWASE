...
bool LuaConsole::tryHandleCommand(const std::string& input)
{
    if (input == ".contexts") {
        ...
        for (size_t i = 0; i < contexts.size(); i++) {
            AddLog("  [%zu] %s%s", i, contexts[i].name.c_str(),
    ...
    if (input == ".active") {
        if (LuaGameEnvironment::IsContextValid()) {
            AddLog("[info] %s", LuaGameEnvironment::GetActiveContextName().c_str());
        }
        else {
    ...
    if (input.rfind(".switch ", 0) == 0) {
        ...
        try {
            ...
            if (LuaGameEnvironment::SelectContext(index)) {
                AddLog("[info] Switched to: %s", LuaGameEnvironment::GetActiveContextName().c_str());
            }
            else {
                AddLog("[error] Invalid context index: %zu", index);
            }
            return true;
        }
        catch (...) {}
        ...
        AddLog("[error] No context matching '%s'", arg.c_str());
        return true;
    }
    ...
    if (input == ".globals") {
        ...
        AddLog("[info] %zu globals:", entries.size());
        for (auto& e : entries) {
            AddLog(("  " + e).c_str());
        }
        return true;
    ...
    if (input.rfind(".list ", 0) == 0) {
        ...
        if (entries.empty()) {
            AddLog("[info] '%s' has no fields or is not a table/userdata", table.c_str());
        }
        else {
            AddLog("[info] %zu fields in '%s':", entries.size(), table.c_str());
            for (auto& e : entries) {
                AddLog(("  " + e).c_str());
            }
        }
...
void LuaConsole::ExecuteCommand(const char* input)
{
    ...
    if (cmd[0] == '.') {
        ...
        AddLog("[error] Unknown command: %s", input);
        return;
    }
    ...
    if (LuaRuntime::loadbuffer(L, cmd.c_str(), cmd.size(), "console") != 0) {
        AddLog("[error] %s", LuaRuntime::tolstring(L, -1, nullptr));
